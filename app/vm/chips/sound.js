...
export default class Sound extends Chip {
  ...
  load (name, path, loop) {
    this._sounds[name] = new Wad({ source: require('file?name=[path]/[name].[ext]!../../_sounds/' + path), loop: loop || false })
    return this
  }

  name (name, random = false) {
    if (random) {
      let c = _.reduce(this._sounds, (r, v, k) => { return r + (_.startsWith(k, name) ? 1 : 0) }, 0)
  ...
  play (name, options = {}, random = false, repeat_min = 1, repeat_max = 1) {
    ...
    while (repeat_max > 0) {
      let s = this._sounds[this.name(name, random)]
      if (s) {
  ...
  stop (name) {
    let s = this._sounds[this.name(name, false)]
    if (s) {
      s.stop()
    }
    return this
  ...
  free (name) {
    delete this._sounds[this.name(name)]
    return this
  }
